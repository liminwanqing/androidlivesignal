...
public class GuardService2 extends FgService {
    private static final String TAG = "keep";

    private final int GuardServiceId1 = 11;
    @Override
    public void onCreate() {
        super.onCreate();
        XK();
        Log.i(TAG, "GuardService2 wait for signal");
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        //提高进程优先级
        createNotification(GuardServiceId1, "GuardService2");
        //绑定建立链接
    ...
    private ServiceConnection connection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName componentName, IBinder iBinder) {
            Log.i(TAG, "connect to guardservice1");
        }
        @Override
        public void onServiceDisconnected(ComponentName componentName) {
            // 断开链接 ,重新启动，重新绑定
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                startForegroundService(new Intent(getApplicationContext(), GuardService1.class));
            } else {
                startService(new Intent(getApplicationContext(), GuardService1.class));
            }
            bindService(new Intent(getApplicationContext(),GuardService1.class),connection, Context.BIND_IMPORTANT);
            Log.i(TAG, "disconnect from guardservice1");
        }
    };
    @Override
    public void onDestroy() {
        Log.i(TAG, "guardservice1 is onDestroy");
